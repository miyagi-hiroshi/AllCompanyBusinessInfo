import type {
  DashboardResponse,
  DashboardServiceComparisonResponse,
} from "@shared/schema/budgetTarget/types";
...
export function useDashboard(fiscalYear: number) {
  return useQuery<DashboardResponse>({
    ...
    queryFn: async () => {
      ...
      const res = await apiRequest("GET", `/api/dashboard?${params}`, undefined);
...
export function useServiceRevenueComparison(fiscalYear: number) {
  return useQuery<DashboardServiceComparisonResponse>({
    ...
    queryFn: async () => {
      ...
      const res = await apiRequest("GET", `/api/dashboard/service-comparison?${params}`, undefined);
      return await res.json();
    },
    enabled: !!fiscalYear,
  });
