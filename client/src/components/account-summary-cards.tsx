...
export function AccountSummaryCards({ summary, isLoading }: AccountSummaryCardsProps) {
  ...
  const totalGlAmount =
    summary.totalGlAmount ?? summary.glSummary.reduce((sum, item) => sum + item.totalAmount, 0);
  const totalOrderAmount =
    summary.totalOrderAmount ??
    summary.orderSummary.reduce((sum, item) => sum + item.totalAmount, 0);
  ...
  const matchRate = totalGlAmount > 0 ? Math.round((matchedAmount / totalGlAmount) * 100) : 0;
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {summary.glSummary.length}科目 /{" "}
              {summary.glSummary.reduce((sum, s) => sum + s.count, 0)}明細
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {summary.orderSummary.length}科目 /{" "}
              {summary.orderSummary.reduce((sum, s) => sum + s.count, 0)}明細
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">突合済み金額の割合</p>
          </CardContent>
        </Card>
      </div>
                  ...
                  .map((diff) => (
                    <TableRow key={diff.accountCode}>
                      ...
                      <TableCell className="text-right font-mono">
                        ¥{diff.glAmount.toLocaleString()}
                      </TableCell>
                      <TableCell className="text-right font-mono">
                        ¥{diff.orderAmount.toLocaleString()}
                      </TableCell>
                      <TableCell
                        className={`text-right font-mono font-bold ${diff.difference > 0 ? "text-blue-600" : "text-red-600"}`}
                      >
