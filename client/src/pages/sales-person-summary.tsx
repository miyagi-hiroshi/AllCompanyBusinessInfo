...
export default function SalesPersonSummaryPage() {
  ...
  const organizedData = useMemo(() => {
    ...
    const salesPersonsFromSummaries = Array.from(
      new Set(summaries.map((s) => s.salesPerson))
    ).sort();
    const availableSalesPersons =
      allSalesPersons.length > 0 ? allSalesPersons : salesPersonsFromSummaries;

    const displaySalesPersons =
      selectedSalesPersons.length > 0 ? selectedSalesPersons : availableSalesPersons;
    ...
    const serviceAnalysisTotals = new Map<
      string,
      {
            ...
            {!hasData ? (
              ...
              <div className="overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="w-[200px] text-xs py-1 px-2 sticky left-0 bg-white">
                        項目
                      </TableHead>
                      {displaySalesPersons.map((salesPerson) => (
                        <TableHead
                          key={salesPerson}
                          className="w-[120px] text-center text-xs py-1 px-2"
                        >
                          {salesPerson}
                        </TableHead>
                      ))}
                      <TableHead className="w-[120px] text-center font-bold text-xs py-1 px-2">
                        合計
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    ...
                    <TableRow className="text-green-700 bg-green-100 font-bold">
                      <TableCell className="text-xs py-1 px-2 sticky left-0 bg-green-100">
                        売上合計
                      </TableCell>
                      {displaySalesPersons.map((salesPerson) => {
                        let total = 0;
                        ...
                        if (serviceMap) {
                          for (const analysisMap of serviceMap.values()) {
                            for (const summary of analysisMap.values()) {
                              total += includeAngleB
                                ? summary.revenueWithAngleB
                                : summary.revenueWithoutAngleB;
                            }
                          }
                        }
                        return (
                          <TableCell
                            key={salesPerson}
                            className="text-right font-mono text-xs py-1 px-2"
                          >
                      ...
                      <TableCell className="text-right font-mono font-bold text-xs py-1 px-2">
                        {formatCurrency(
                          includeAngleB
                            ? grandTotal.revenueWithAngleB
                            : grandTotal.revenueWithoutAngleB
                        )}
                      </TableCell>
                    </TableRow>
                    ...
                    <TableRow className="text-red-700 bg-red-100 font-bold">
                      <TableCell className="text-xs py-1 px-2 sticky left-0 bg-red-100">
                        売上原価合計
                      </TableCell>
                      {displaySalesPersons.map((salesPerson) => {
                        let total = 0;
                        ...
                        if (serviceMap) {
                          for (const analysisMap of serviceMap.values()) {
                            for (const summary of analysisMap.values()) {
                              total += includeAngleB
                                ? summary.costOfSalesWithAngleB
                                : summary.costOfSalesWithoutAngleB;
                            }
                          }
                        }
                        return (
                      ...
                      <TableCell className="text-right font-mono font-bold text-xs py-1 px-2">
                        {formatCurrency(
                          includeAngleB
                            ? grandTotal.costOfSalesWithAngleB
                            : grandTotal.costOfSalesWithoutAngleB
                        )}
                      </TableCell>
                    </TableRow>
                    ...
                    <TableRow className="text-blue-700 bg-blue-100 font-bold">
                      <TableCell className="text-xs py-1 px-2 sticky left-0 bg-blue-100">
                        粗利
                      </TableCell>
                      {displaySalesPersons.map((salesPerson) => {
                        let total = 0;
                        ...
                        if (serviceMap) {
                          for (const analysisMap of serviceMap.values()) {
                            for (const summary of analysisMap.values()) {
                              total += includeAngleB
                                ? summary.grossProfitWithAngleB
                                : summary.grossProfitWithoutAngleB;
                            }
                          }
                        }
                        return (
                      ...
                      <TableCell className="text-right font-mono font-bold text-xs py-1 px-2">
                        {formatCurrency(
                          includeAngleB
                            ? grandTotal.grossProfitWithAngleB
                            : grandTotal.grossProfitWithoutAngleB
                        )}
                      </TableCell>
                    </TableRow>
                    ...
                    {SERVICE_ORDER.map((serviceType) => {
                      ...
                      // 分析区分を順序通りにソート
                      const sortedAnalysisTypes = ANALYSIS_TYPE_ORDER.filter((at) =>
                        analysisTypes.has(at)
                      );
                      ...
                      return (
                        <React.Fragment key={serviceType}>
                          ...
                          <TableRow className="bg-gray-100">
                            <TableCell
                              colSpan={displaySalesPersons.length + 2}
                            ...
                            return (
                              <React.Fragment key={key}>
                                ...
                                <TableRow className="bg-gray-50">
                                  <TableCell
                                    colSpan={displaySalesPersons.length + 2}
                                    className="text-xs font-medium py-1 px-2 pl-4"
                                  >
                                    {analysisType}
                                  </TableCell>
                                </TableRow>
                                ...
                                <TableRow className="text-green-700 bg-green-50">
                                  <TableCell className="text-xs py-1 px-2 sticky left-0 bg-green-50 pl-8">
                                    売上
                                  </TableCell>
                                  {displaySalesPersons.map((salesPerson) => {
                                    const serviceMap = dataMap.get(salesPerson);
                                ...
                                <TableRow className="text-red-700 bg-red-50">
                                  <TableCell className="text-xs py-1 px-2 sticky left-0 bg-red-50 pl-8">
                                    売上原価
                                  </TableCell>
                                  {displaySalesPersons.map((salesPerson) => {
                                    const serviceMap = dataMap.get(salesPerson);
                                ...
                                {showGrossProfit && (
                                  <TableRow className="text-blue-700 bg-blue-50">
                                    <TableCell className="text-xs py-1 px-2 sticky left-0 bg-blue-50 pl-8">
                                      粗利
                                    </TableCell>
                                    {displaySalesPersons.map((salesPerson) => {
                                      const serviceMap = dataMap.get(salesPerson);
