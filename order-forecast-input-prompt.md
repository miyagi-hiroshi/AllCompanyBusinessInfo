...
- 入力データに複数プロジェクトが混在している場合、**プロジェクトフィルタを切り替える前に必ず保存すること**。
- プロジェクトフィルタを変更すると表示データが切り替わり、**未保存の新規行（`temp-` 行）や未保存の編集内容が画面から消失する可能性がある**。
- 必ず以下の順序で作業する：
  1. プロジェクトフィルタを対象プロジェクトに設定
  2. 新規行追加 → 入力（必要な行数分）
  3. **`Ctrl+Enter` で保存**し、「保存しました」のトースト通知を確認
  4. 次のプロジェクトへフィルタを切り替え（1へ戻る）
...
- 複数プロジェクトにまたがる入力は、**「1プロジェクト分の入力 → 保存 → 次のプロジェクトへフィルタ変更」**を必ず繰り返す。
- **保存前にプロジェクトフィルタを変更しない**（未保存データが消失する可能性があるため）。
