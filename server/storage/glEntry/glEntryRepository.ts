  ...
  async findUnmatched(period?: string): Promise<GLEntry[]> {
    if (period) {
      return await db
        .select()
        .from(glEntries)
        .where(and(eq(glEntries.reconciliationStatus, "unmatched"), eq(glEntries.period, period)));
    }
  ...
  async findMatched(period?: string): Promise<GLEntry[]> {
    if (period) {
      return await db
        .select()
        .from(glEntries)
        .where(and(eq(glEntries.reconciliationStatus, "matched"), eq(glEntries.period, period)));
    }
