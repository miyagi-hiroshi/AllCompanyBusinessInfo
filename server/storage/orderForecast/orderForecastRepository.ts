...
export class OrderForecastRepository {
  ...
  async findByPeriod(period: string): Promise<OrderForecast[]> {
    return await db
      .select()
      .from(orderForecasts)
      .where(eq(orderForecasts.accountingPeriod, period));
  }
  ...
  async findUnmatched(period?: string): Promise<OrderForecast[]> {
    ...
    return await db
      .select()
      .from(orderForecasts)
      .where(eq(orderForecasts.reconciliationStatus, "unmatched"));
  }
  ...
  async findMatched(period?: string): Promise<OrderForecast[]> {
    if (period) {
      return await db
        ...
        .from(orderForecasts)
        .where(
          and(eq(orderForecasts.reconciliationStatus, "matched"), eq(orderForecasts.period, period))
        );
    }
    ...
    return await db
      .select()
      .from(orderForecasts)
      .where(eq(orderForecasts.reconciliationStatus, "matched"));
  }
  ...
  > {
    ...
    const summaryMap = new Map<
      string,
      { revenue: number; costOfSales: number; sgaExpenses: number }
    >();
    ...
    for (const row of result) {
      ...
      if (
        accountingItem === "保守売上" ||
        accountingItem === "ソフト売上" ||
        accountingItem === "商品売上" ||
        accountingItem === "消耗品売上" ||
      ...
      // 販管費（通信費、消耗品費、支払保守料、外注加工費、その他調整経費）
        ...
        accountingItem === "通信費" ||
        accountingItem === "消耗品費" ||
        accountingItem === "支払保守料" ||
        accountingItem === "外注加工費" ||
  ...
  /** プロジェクト分析の集計対象計上科目（getProjectAnalysisSummary と一致させる） */
  private static readonly REVENUE_ITEMS = [
    "保守売上",
    "ソフト売上",
    "商品売上",
    "消耗品売上",
  ...
  private static readonly COST_OF_SALES_ITEMS = [
    "期首製品棚卸高",
    "期首商品棚卸高",
    "仕入高",
    "期末製品棚卸高",
  ...
  private static readonly SGA_ITEMS = [
    "通信費",
    "消耗品費",
    "支払保守料",
    "外注加工費",
  ...
  async getProjectAnalysisDetailLines(
    ...
    const accountingItems =
      category === "revenue"
        ? OrderForecastRepository.REVENUE_ITEMS
        : category === "costOfSales"
          ? OrderForecastRepository.COST_OF_SALES_ITEMS
          : OrderForecastRepository.SGA_ITEMS;
    ...
    const rows = await db
      ...
      .where(
        and(
          eq(orderForecasts.projectId, projectId),
          sql`${orderForecasts.accountingPeriod} >= ${startPeriod}`,
          sql`${orderForecasts.accountingPeriod} <= ${endPeriod}`,
          inArray(orderForecasts.accountingItem, accountingItems)
        )
      )
      .orderBy(asc(orderForecasts.accountingPeriod), asc(orderForecasts.accountingItem));
