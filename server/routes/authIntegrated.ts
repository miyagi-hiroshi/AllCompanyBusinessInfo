...
import { z } from 'zod';
import { getExistingUserByEmail, getExistingEmployeeByUserId } from '../storage/existing';
import { isAuthenticatedIntegrated } from '../middleware/authIntegrated';
...
/**
 * 統合認証ログインAPI
 * POST /api/auth-integrated/login
 */
...
/**
 * 統合認証ログアウトAPI
 * POST /api/auth-integrated/logout
 */
router.post('/logout', isAuthenticatedIntegrated, async (req: Request, res: Response) => {
  try {
...
/**
 * 統合認証ユーザー情報取得API
 * GET /api/auth-integrated/me
 */
router.get('/me', isAuthenticatedIntegrated, async (req: Request, res: Response) => {
  try {
...
/**
 * CSRFトークン取得API
 * GET /api/auth-integrated/csrf-token
 */
